...
Raw LendingClub Data (.csv)
            ...
            v
+--------------------------------------------------------+
| 1. SQL ETL Pipeline (data_loader.py & .sql)            |
|    - DuckDB executes SQL natively on the raw CSV       |
...
Data Manipulation & Storage: DuckDB (SQL Engine), Pandas, NumPy, PyArrow (Parquet)
...
|-- sql/
|   `-- clean_lending_club.sql           # DuckDB SQL transformation query
|-- src/
|   |-- check_data.py                    # Data ingestion sanity check
|   |-- data_loader.py                   # DuckDB SQL execution and Parquet export
...
4. Download the LendingClub Dataset
Ensure your kaggle.json token is configured in ~/.kaggle/ (or C:\Users\<Username>\.kaggle\), then run:
kaggle datasets download -d wordsforthewise/lending-club -p data/raw --unzip

Execution Pipeline

Run the scripts in sequential order:
# Step 1: Execute SQL ETL pipeline via DuckDB
